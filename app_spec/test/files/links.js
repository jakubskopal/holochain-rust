...
module.exports = scenario => {
    ...
    scenario('delete_post', async (s, t) => {
      ...
      scenario('get_links_and_load with a delete_post', async (s, t) => {
        const { alice } = await s.players({alice: one}, true)

        //create post
        const alice_create_post_result = await alice.callSync("app", "blog", "create_post",
          { "content": "Posty", "in_reply_to": "" }
        )

        const alice_get_post_result1 = await alice.callSync("app", "blog", "my_posts_with_load",
          { "tag": null }
        )
        ...
        await alice.callSync("app", "blog", "delete_post", { "content": "Posty", "in_reply_to": "" })

        const alice_get_post_result2 = await alice.callSync("app", "blog", "my_posts_with_load",
          { "tag": null }
        )
      ...
      scenario('tagged link validation', async (s, t) => {
        const { alice } = await s.players({alice: one}, true)
        const result1 = await alice.callSync("app", "blog", "create_tagged_post", {
        ...
        const getResult = await alice.callSync("app", "blog", "my_posts", {})
        t.equal(getResult.Ok.links.length, 0)
      })

      scenario('create_post with bad reply to', async (s, t) => {
        const { alice } = await s.players({alice: one}, true)
        const content = "Holo world"
        const in_reply_to = "bad"
        const params = { content, in_reply_to }
        const result = await alice.call("app", "blog", "create_post", params)
      ...
      scenario('posts_by_agent', async (s, t) => {
        const { alice } = await s.players({alice: one}, true)

        const agent = "Bob"
        const params = { agent }

        const result = await alice.call("app", "blog", "posts_by_agent", params)

        t.deepEqual(result.Ok, { links: [] })
      })

      scenario('my_posts', async (s, t) => {
        const { alice } = await s.players({alice: one}, true)
        ...
        const result = await alice.call("app", "blog", "my_posts", {})

        t.equal(result.Ok.links.length, 2)
      })

      scenario('my_posts_immediate_timeout', async (s, t) => {
        const { alice } = await s.players({alice: one}, true)

        await alice.call("app", "blog", "create_post",
          { "content": "Holo world", "in_reply_to": "" }
        )

        const result = await alice.call("app", "blog", "my_posts_immediate_timeout", {})
